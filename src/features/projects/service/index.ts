...
import { ETaskStatus, TProject } from "@/features/types"
import { endOfMonth, startOfMonth, subMonths } from "date-fns"
...
const app = new Hono<{ Variables: AppVariables }>()
	...
	.get("/:projectId", authSessionMiddleware, async (c) => {
		try {
			...
			let project;
			try {
				project = await databases.getDocument<TProject>(DATABASE_ID, PROJECTS_ID, projectId)
			} catch (error) {
	...
	.patch(
		...
		async (c) => {
			try {
				...
				let existingProject;
				try {
					existingProject = await databases.getDocument<TProject>(DATABASE_ID, PROJECTS_ID, projectId)
				} catch (error) {
					console.error("Failed to fetch project:", error)
					return c.json({ error: "Project not found" }, 404)
				}
	...
	.delete("/:projectId", authSessionMiddleware, async (c) => {
		try {
			...
			return c.json({ data: { $id: existingProject.$id } }, 200)
		} catch (error) {
