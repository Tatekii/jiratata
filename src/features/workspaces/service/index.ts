...
const app = new Hono<{ Variables: AppVariables }>()
  ...
  .get("/:workspaceId", authSessionMiddleware, async (c) => {
    try {
      ...
      return c.json({ data: member.workspaceId })
    } catch (error) {
  ...
  // 创建新工作区
  .post("/", authSessionMiddleware, localeMiddleware, localeValidatorMiddleware("json", buildCreateWorkspaceSchema), async (c) => {
    try {
      const user = c.get("user")
  ...
  // 更新工作区
  .patch("/:workspaceId", authSessionMiddleware, localeMiddleware, localeValidatorMiddleware("json", buildUpdateWorkspaceSchema), async (c) => {
    try {
      const user = c.get("user")
      const { workspaceId } = c.req.param()
